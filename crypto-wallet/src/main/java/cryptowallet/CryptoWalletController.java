...
public class CryptoWalletController {
    ...
    public ResponseEntity<?> conversion(@RequestBody CryptoWalletDto wallet) {
        ...
        switch (wallet.getFrom().toLowerCase()) {
            ...
            case "xrp": {
                if (repo.findByEmail(wallet.getEmail()).getXrp().compareTo(wallet.getFromValue()) == -1)
		            return ResponseEntity.status(400).body("You don't have enough " + wallet.getFrom() + " for the exchange");
                else {
                    walletDb.setXrp(repo.findByEmail(wallet.getEmail()).getXrp().subtract(wallet.getFromValue()));
                }
                break;
            }
        ...
        switch (wallet.getTo().toLowerCase()) {
            ...
            case "xrp": {
                walletDb.setXrp(repo.findByEmail(wallet.getEmail()).getXrp().add(wallet.getToValue()));
                break;
            }
        ...
        CryptoWalletResponseDto responseDto = new CryptoWalletResponseDto(
            walletDb.getEmail(), 
            walletDb.getBtc(), walletDb.getEth(), walletDb.getXrp(), 
